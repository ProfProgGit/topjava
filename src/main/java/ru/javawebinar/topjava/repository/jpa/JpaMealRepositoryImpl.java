...
public class JpaMealRepositoryImpl implements MealRepository {
    ...
    public Meal save(Meal meal, int userId) {
        if (meal.isNew()) {
            ...
            if (em.createNamedQuery(Meal.UPDATE).setParameter("id", meal.getId())
                    ...
                    .executeUpdate() == 1) {
                return this.get(meal.getId(), userId);
            } else {
                return null;
            }
        }
    ...
    public Meal get(int id, int userId) {
        try {
            return em.createNamedQuery(Meal.MEAL_BY_ID, Meal.class)
                    .setParameter("id", id)
                    .setParameter("userId", userId)
                    .getSingleResult();
    ...
    public List<Meal> getBetween(LocalDateTime startDate, LocalDateTime endDate, int userId) {
        return em.createNamedQuery(Meal.ALL_BY_USER_BETWEEN_DATES, Meal.class)
                .setParameter("userId", userId)
